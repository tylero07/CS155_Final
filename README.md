...
- Prompt-driven decision points (Questions 1–4)  
...
1. **Assemble** your source file (e.g. `oregon_trail.asm`) twice:
   - Once with `.ORIG x3000` to compile the main program.
   - Again with `.ORIG x3800` to compile the custom TRAP x38 routine.
2. **Load** both object files into your simulator:
   - Program at address `x3000`
   - Trap routine at address `x3800`
3. **Run** starting at `x3000`.  
   ...
   5. Present a final update and halt.
...
| **UPDATE_PARTY**        | Adds TRAP‐returned random number (R1) to party size                                      |
